"""
Naive RAG Result Saver

Handles saving and loading of Naive RAG QA results in JSONL format.
"""
import json
import os
...
class NaiveRAGSaver:
    """Save and load Naive RAG results"""
    ...
    def save_answer(answer_data: Dict[str, Any], model_name: str, dataset_name: str) -> None:
        ...
        """
        answer_file = PathConfig.get_naive_rag_path(model_name, dataset_name)
    ...
    def save_retrieval(retrieval_data: Dict[str, Any], model_name: str, dataset_name: str) -> None:
        ...
        Args:
            retrieval_data: Dictionary with keys: id, chunks (list of retrieved chunks)
            model_name: Name of the LLM model
            dataset_name: Name of the dataset
        """
        answer_file = PathConfig.get_naive_rag_path(model_name, dataset_name)
    ...
    @staticmethod
    def load_retrievals(model_name: str, dataset_name: str) -> Optional[List[Dict[str, Any]]]:
    ...
    @staticmethod
    def update_answers(updated_answers: Dict[str, str], model_name: str, dataset_name: str) -> int:
        """Update specific answers in the answer file (for retrying null answers)

        Args:
            updated_answers: Dictionary mapping id -> new rag_answer
            model_name: Name of the LLM model
            dataset_name: Name of the dataset
        ...
        # Update the specified answers
        updated_count = 0
        for answer in answers:
            q_id = answer.get('id')
            if q_id in updated_answers:
                answer['rag_answer'] = updated_answers[q_id]
                updated_count += 1
        ...
        print(f"Updated {updated_count} answers in: {answer_file}")
...
if __name__ == "__main__":
    ...
    # Save
    NaiveRAGSaver.save_all(test_results, "deepseek-chat", "test")

    # Load
    loaded = NaiveRAGSaver.load_answers("deepseek-chat", "test")
